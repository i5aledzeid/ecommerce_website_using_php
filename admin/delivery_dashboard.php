...
if(isset($_POST['updatedata1'])) {
    ...
    $update_payment = $conn->prepare("UPDATE `order_store` SET delivery_price = ?, payment_status = ? WHERE id = ?");
    $update_payment->execute([$defualt_delivery_price, $payment_status, $id]);
...
<section class="dashboard">
    ...
    <h1 class="heading" style="font-size: 16px;">
        ...
        <a href="delivery_dashboard.php?status=all" style="text-decoration: none; color: black;">الكل</a>
        <?php } ?>
        |
        ...
        ?>
        <?php if(!isset($delivery_id) || isset($admin_id)){ ?>
        <a href="delivery_dashboard.php?status=all" style="background: red; padding: 8px; border-radius: 16px; color: white; text-decoration: underline;">
            الكل
            <?php echo '(' .$number_of_order_store. ')'; ?>
        </a>
        <?php } ?>
   ...
   <div class="box-container" style="direction: rtl;">
    ...
    <?php
        $i = 1;
        if ($status == 'all') {
            if(!isset($delivery_id) && isset($admin_id)){
                        ...
                        <div class="row">
                            <div class="col">
                                <p style="font-size: 12px; padding: 0; direction: ltr;">
                                    ...
                                    <?php echo 'SID: ' . $fetch_store_orders['sid']; ?>
                                </p>
                            </div>
                        </div>
                        <div class="row">
