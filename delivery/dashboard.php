...
if(isset($_POST['updatedata'])) {
    ...
    $update_payment_order = $conn->prepare("UPDATE `orders` SET payment_status = ? WHERE id = ?");
    $update_payment_order->execute([$payment_status, $id]);
...
<section class="dashboard">
    ...
    <h1 class="heading">
        الطلبات
        <a href="dashboard.php" style="color: red; text-decoration: underline;">المتاحة</a>
        |
        <a href="rdashboard.php">المحجوزة</a>
        |
        <a href="cdashboard.php">المكتملة</a>
    </h1>
   ...
   <div class="box-container" style="direction: rtl;">
    ...
    <?php
        $i = 1;
        $select_store_orders = $conn->prepare("SELECT * FROM `store_orders` WHERE payment_status='pending' OR payment_status='unreservation'"); 
